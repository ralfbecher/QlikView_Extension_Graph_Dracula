...
Graph.Renderer.defaultRenderFunc = function(r, node) {
    ...
    var ellipse = r.ellipse(0, 0, 7.5, 5).attr({fill: node.fill || color, stroke: node.stroke || color, "stroke-width": 2});
    /* set DOM node ID */
    ellipse.node.id = node.label || node.id;
    shape = r.set().
        push(ellipse).
        push(r.text(0, 18, node.label || node.id));
    return shape;
}
...
Graph.Renderer.Raphael.prototype = {
    ...
    drawEdge: function(edge) {
        ...
        edge.connection.fg.show().toBack();
        edge.connection.bg && edge.connection.bg.show().toBack();
